...
- `data/videos.json`: the six AIMLArchives videos on the `/content/` video gallery.
- `data/projects.json`: curated public personal repositories on the projects page.
- `static/images/asad-linkedin.jpg`: the current LinkedIn portrait, retrieved from the user's profile on 18 September 2026. This local image also supplies the social preview.
...
- `layouts/partials/ai-orbit.html`: the user's requested orbit illustration, reduced to a 48 × 40px mark beside the name in the shared header. The decorative SVG retains the orbits, nodes, and central “ai.”, omits labels and texture that become illegible at this size, uses the dark theme colors, and has no animation. The surrounding home link supplies its accessible name.
- `assets/css/site.css`: the active stylesheet. The older theme SCSS is retained but is not loaded by the custom base layout.

The site defaults to a near-black theme with warm off-white text, amber links, serif headings, and plain article and project lists. CSS and document metadata both declare the dark color scheme, independent of the operating-system preference. Code, tables, selection colors, and browser chrome follow the same palette; printing uses dark text on white. The homepage has a prominent LinkedIn portrait and a readable introduction to end-to-end AI/ML work: training, fine-tuning, distillation, evaluation, and deployment, including edge devices. The portrait is 320px on desktop and 200px on phones; the introduction uses larger text and a stacked layout on narrow screens. Writing, videos, and projects live on separate pages, as requested by the user; do not collect their listings on the homepage. Blog & writing, Videos, Projects, and About stay visible in the main navigation at every screen size. The user wants the clarity of George Hotz, Andrej Karpathy, and Lilian Weng’s sites with an original visual identity. The small orbit illustration is explicitly requested and fits the otherwise restrained layout. Avoid marketing headlines, badges, and repeated calls to action. No client-side JavaScript or font download is required. Video previews link to YouTube without loading embedded players.
